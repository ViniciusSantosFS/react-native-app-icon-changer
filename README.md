...
```bash
npm i @vinicius_santos/react-native-app-icon-changer
```

```bash
yarn add @vinicius_santos/react-native-app-icon-changer
```
...
## iOS System Alert and App Store Review

iOS shows a native system alert when an app changes its alternate icon. This alert is generated by the operating system, not by this library, and there is no supported public API for disabling it.

Do not suppress or bypass this alert in App Store builds. Suppressing it typically requires private APIs or modifying expected native iOS behavior, which can cause App Store rejection. Apple states that apps altering or disabling native user interface elements or behaviors may be rejected.

- [Apple App Store Review Guidelines: Alternate App Icons](https://developer.apple.com/app-store/review/guidelines/#alternate-app-icons)
